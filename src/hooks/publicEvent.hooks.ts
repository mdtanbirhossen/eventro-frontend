...
export function useJoinPublicEvent() {
  ...
  return useMutation({
    ...
    onSuccess: (res) => {
      ...
      const paymentUrl = (res.data as { payment?: { paymentUrl?: string } })
        ?.payment?.paymentUrl;
      ...
      // refetch joined events in user dashboard if cached
      queryClient.refetchQueries({ queryKey: ["my-joined-events"] });
    },
    onError: (error: Error) => {
